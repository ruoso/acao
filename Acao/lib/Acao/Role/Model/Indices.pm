...
my $controle = XML::Compile::Schema->new( Acao->path_to('schemas/indexhint.xsd') );
...
sub extract_xml_keys {
    ...
    my $xquery = 'declare namespace ns="http://schemas.fortaleza.ce.gov.br/acao/dossie.xsd";
                  ...
                  for $x in collection("'.$id_volume.'")/ns:dossie[ns:controle="'.$controle.'"]
                       /ns:doc/dc:documento[dc:id="'.$id_documento.'"]/dc:documento/dc:conteudo
                  return ('.$xqueryret.')';
...
sub get_nm_volume {
    ...
    my ($id_volume) = @_;
    my $xquery = 'declare namespace ns = "http://schemas.fortaleza.ce.gov.br/acao/volume.xsd"; 
                  for $x in collection("volume")/ns:volume[ns:collection="'.$id_volume.'"]/ns:nome 
                  return $x/text()';
    $self->sedna->execute($xquery);
