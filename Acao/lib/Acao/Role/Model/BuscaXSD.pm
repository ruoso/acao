...
sub produce_expr {
    my ($self, $nsprefix, $path_form, $oper, $valor, $xpath_prefix, $type)= @_;
    $xpath_prefix ||= '';
    $type ||= 'where';
    my $tipo_operador;
    my $operador;
    ...
    given ($type) {
        when ('where') {
            my $va = $xpath_prefix.$self->produce_xpath($nsprefix, $path_form);
            my $vb = $valor;
            return $self->produce_operador($classe_operador, $tipo_operador, $operador, $va, $vb);
        }
        when ('xpfilter') {
